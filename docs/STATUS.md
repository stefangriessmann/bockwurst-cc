...
- **Increment 3 ✅ (2026-07-07):** **6-Points-Sektion** (`bw-sixp`). Dunkler Banner nach Handoff: 6-Points-White-Logo (via `f:uri.resource`), Botschafter-Zeile, H2 „Rund um **Mallorca**" (gelb/`--yellow`), Text, **Live-Countdown** bis 2027-05-14 09:00 (`initCountdown` in `bockwurst-home.js`, läuft unabhängig von reduced-motion; **No-JS-Fallback** „14. Mai 2027 · Mallorca"), Volt-CTA → `6pointschallenges.com/product/mallorca-may-2027/`. Foto-Spalte = **Gradient-Fallback** (Mallorca-Foto fehlt noch). Verifiziert: HTTP 200, Logo/JS 200, keine Exceptions.
  - **Mallorca-Foto einbauen (wenn vorhanden):** Foto nach `fileadmin/user_upload/` laden, dann in `Home.html` am `.bw-sixp-photo`-Div `style="background-image:url('/fileadmin/user_upload/<datei>')"` setzen (der Gradient in `bockwurst-home.css` bleibt als Fallback darunter).
- **Offen je Sektion:** ~~Touren-Grid~~ ✅; ~~6 Points~~ ✅ (Foto offen); **Events** → Daten aus `sport-events` (Integration) oder Link-out; **Shorts** → **4 Short-IDs**; **Über** → **Text + Foto**. Pro Tour weiterhin offen: Hero-Foto (→ füllt automatisch die Grid-Karte statt Gradient), Highlights, Bericht, Bayern-YT-IDs.
